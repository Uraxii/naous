...
func set_shard_manager_url(host: String, port: int) -> void:
    ...
    logger.info("Shard manager URL set to: %s" % shard_manager_url)
...
func _on_heartbeat_response(response_code: int, response_text: String) -> void:
    ...
    if response_code == 200:
        ...
        logger.warn("Shard not found in manager - may have been removed")
        # Could trigger self-shutdown here
    else:
        logger.error("Heartbeat failed: %d - %s" % [response_code, response_text])


func _on_stats_response(response_code: int, response_text: String) -> void:
    """Handle stats update response."""
    if response_code == 200:
        logger.info("Stats update successful")
    elif response_code == 404:
        logger.warn("Shard not found for stats update")
    else:
        logger.error("Stats update failed: %d - %s" % [response_code, response_text])


func _on_shutdown_response(response_code: int, response_text: String) -> void:
    """Handle shutdown confirmation response."""
    if response_code == 200:
        logger.info("Shutdown confirmed by manager")
    else:
        logger.warn("Shutdown confirmation failed: %d" % response_code)


func _on_startup_response(response_code: int, response_text: String) -> void:
    """Handle startup registration response."""
    if response_code == 200:
        logger.success("Shard startup registered successfully")
    else:
        logger.error("Startup registration failed: %d - %s" % 
...
func _on_dungeon_creation_response(response_code: int, response_text: String) -> void:
    ...
    if response_code == 200:
        ...
        if error == OK:
            ...
            logger.success("Dungeon created: %s" % shard_info.get("shard_id", "unknown"))
            # Emit signal with connection info for the requesting player
            Globals.signal_bus.dungeon_created.emit(shard_info)
        else:
            logger.error("Failed to parse dungeon creation response")
    else:
        logger.error("Dungeon creation failed: %d - %s" % 
                  [response_code, response_text])
...
func _handle_manager_commands(commands: Array) -> void:
    ...
    for command in commands:
        match command.get("type", ""):
            "shutdown":
                logger.info("Received shutdown command from manager")
                _execute_shutdown_command(command)
            "update_settings":
                logger.info("Received settings update from manager")
                _execute_settings_update(command)
            "player_limit_change":
                logger.info("Received player limit change from manager")
                _execute_player_limit_change(command)
            _:
                logger.warn("Unknown command from manager: %s" % command.get("type", ""))
...
func _execute_shutdown_command(command: Dictionary) -> void:
    ...
    logger.info("Shutting down in %d seconds" % grace_period)
...
func _execute_settings_update(command: Dictionary) -> void:
    ...
    logger.info("Updating settings: %s" % new_settings)
...
func _execute_player_limit_change(command: Dictionary) -> void:
    ...
    logger.info("Changing player limit to: %d" % new_limit)
...
func _server_shutdown_notification(grace_period: int) -> void:
    ...
    logger.info("Server shutting down in %d seconds" % grace_period)
