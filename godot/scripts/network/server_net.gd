...
var peer_actors: Dictionary[int, int] = {  }
...
func fetch_my_actor(promise_id: int) -> void:
    ...
    var actor_id = peer_actors.get(sender_peer_id, Actor.INVALID_ID)

    if not actor_id:
        return

    var resp := actor_db.find(actor_id)
    respond.rpc_id(
        sender_peer_id,
        promise_id,
        resp.stats,
...
func fetch_player_data(promise_id: int, user_id: int) -> void:
    ...
    respond.rpc_id(sender_peer_id, promise_id, data)


@rpc("any_peer", "call_remote", "reliable")
func set_player_data(actor_id: int, data: Dictionary) -> void:
...
func _on_player_connected(peer_id: int) -> void:
    ...
    peer_actors[peer_id] = new_player.id
